...
/*  Figma adds the "Check your inbox" card + Change email / Skip links,  */
...
export default function EnflowVerifyEmailStep({
  ...
  function handleContinue() {
    if (isComplete) {
      ...
      onContinue?.({ verified: false, skipped: true });
    }
  }

  const canContinue = isComplete || skipped;
      ...
      <div className="efw-shell">
        ...
        <div className="efw-otp-card">
          ...
          <div className="efw-otp-links">
            ...
            <button className="efw-otp-link warn" type="button" onClick={handleSkip}>Skip for now</button>
          </div>

          {resentNote && <p className="efw-note">New code sent to {email}</p>}
          {skipped && <p className="efw-note">Skipped — we'll remind you to verify later.</p>}
        </div>
